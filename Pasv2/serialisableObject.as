...
package Pasv2{
	...
	public class serialisableObject extends Object {
		public static var instance = new serialisableObject();
		...
		//serialisableObject.RefsContainer[root.newSprite] = "this.newSprite";
		//if your class extends serialisableObject, define the absoluteRefString before the super function
		// and by default, the serialisableObject constructor will put an entry into RefsCOntainer
		...
		var iteratedObjects=serialisableObject.globalobj.iteratedObjects;
		...
		public function serialisableObject(... rest) {
			...
			if (this.absoluteRefString != null) {
				serialisableObject.RefsContainer[this] = this.absoluteRefString;
			}
			// constructor code
		...
		public static function addRefEntry(obj:Object,refStr:String){
			Pasv2.serialisableObject.RefsContainer[obj]=refStr;
		}
		...
		public static function addFeederEntry(obj:Object,feederObj:Pasv2.feederObject){
			var ref:Object=obj;
			Pasv2.serialisableObject.feederDictionary[obj]=feederObj;
		}
		//generate feeder object for non serialisableObjects
		public static function generateFeederObject(prop:Object):Pasv2.feederObject {
			var paramA:Array = [];
		var feederdict1:Pasv2.feederObject= serialisableObject.feederDictionary[prop];
			var feederdict:Pasv2.feederObject = serialisableObject.feederDictionary[prop.constructor];
		...
		public static function initAbsoluteRefTags():void {
			...
			var refCont = serialisableObject.RefsContainer;
			var refTags = serialisableObject.globalobj.refTags;
		...
		public static function resetGlobalObj():void {
			serialisableObject.globalobj = {};
			serialisableObject.globalobj.is_Empty=function(){
			var numberprops:Number=0;
			var iteratedObjects=serialisableObject.globalobj.iteratedObjects;
			...
			serialisableObject.globalobj.codeBlock = "";
			serialisableObject.globalobj.localIDs = {};
			serialisableObject.globalobj.importDefs = {};
			//contains the objects which the serialiser has iterated over
			serialisableObject.globalobj.iteratedObjects = new Dictionary(false);
			serialisableObject.globalobj.refTags = new Dictionary(false);
		}
		...
		public static function isTop():Boolean {
			...
			//or as part of a recursive serialise/serialisableObject.serialiseChildren call
		...
		*/
		...
		public static function abbrevLocalRef(referencestr:String) {
			...
			for(var i=dotarray.length-1;i>=dotarray.length-serialisableObject.AbbrevDepth;i--){;
	...
	public function serialise(referenceStr:String,includeVar:Boolean =false,isTopParam=false,thisObject:Object=null,feederObject:Pasv2.feederObject = null) {
		...
		for (var p=0; p<paramsArrayy.length; p++) {
			...
			if (typeof param != "object") {
				...
				if (param.hasOwnProperty("absoluteRefString") && param.absoluteRefString != null) {
					...
					//continue;
				} else if (serialisableObject.RefsContainer[param]!=null) {
					//an aobsolute ref cannot be found, but RefsContainer contains a ref to the param
					paramRefStr = serialisableObject.RefsContainer[param];
					paramsStr +=  commaStr + paramRefStr;
				...
				if (paramRefStr=="") {
					...
					var referenceOfIteratedObj:String = serialisableObject.getIteratedReference(param);
					var localInitStr2:String = "";
					if (referenceOfIteratedObj==null) {
						var localID = serialisableObject.generateLocalID(referenceStr);
						var paramRefStr = localID + "_arg" + p;

						//paramRefStr = serialisableObject.generateLocalID(referenceStr + "." + paramName);
						...
						if (param.hasOwnProperty("serialise")&&param.serialise is Function) {
							...
							//before this function is called!
							  ...
							  var feederObjp:Pasv2.feederObject= Pasv2.serialisableObject.generateFeederObject(param);
                     localInitStr2 = Pasv2.serialisableObject.serialiseObject(param,paramRefStr,true,false,feederObjp);
	    ...
	    if(this.absoluteRefString==null){
		if(Pasv2.serialisableObject.RefsContainer[this]!=null){
			var importPasV2Str:String="";
			if(globalobj.importDefs["Pasv2.serialisableObject"]==null){
				globalobj.importDefs["Pasv2.serialisableObject"]=true;
				importPasV2Str="\r import Pasv2.serialisableObject;"
			}
			...
			refAssignStr=importPasV2Str+"\r Pasv2.serialisableObject.addRefEntry("+referenceStr+",'"+referenceStr+"');";
		   }
	       }
	...
	//a global function for non serialisableObjects
...
if(feederObject ==null){
	feederObject = Pasv2.serialisableObject.generateFeederObject(thisObject);
}
		//thisObject = invoke serialise as part of a call of another object, for non serialisable objects.
		var isTop:Boolean = serialisableObject.isTop();
		if (isTop==true&&isTopParam!=false) {
		...
		if(Pasv2.serialisableObject.RefsContainer[thisObject]!=null){
			var importPasV2Str:String="";
			if(globalobj.importDefs["Pasv2.serialisableObject"]==null){
				globalobj.importDefs["Pasv2.serialisableObject"]=true;
				importPasV2Str="\r import Pasv2.serialisableObject;"
...
if (referenceOfIteratedObj==null) {
paramrefStr = serialisableObject.generateLocalID(referenceStr + ".param"+p);
globalobj.localIDs[paramrefStr] = param;
...
} else if (serialisableObject.RefsContainer[dynamicVar]!=null) {
//an aobsolute ref cannot be found, but RefsContainer contains a ref to the param
dynamicRefStr = serialisableObject.RefsContainer[dynamicVar];
footer +=  "" + referenceStr + "." + dynamicVarName + " = " + dynamicRefStr + ";";
}
//if the absolute reference of the dynamic var includes referenceStr as a parent
//the dynamic var is a child of this object, and not a reference to someplace else
...
var referenceOfIteratedObj2:String = serialisableObject.getIteratedReference(dynamicVar);
var localInitStr2:String = "";
if (referenceOfIteratedObj2==null) {
dynamicRefStr = serialisableObject.generateLocalID(referenceStr + "." +dynamicVarName);
